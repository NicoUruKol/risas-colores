...
export default function AdminGoogleReviews() {
    ...
    const validate = () => {
        ...
        if (!isValidHttpUrl(form.authorPhotoUrl)) {
        next.authorPhotoUrl = "URL inválida (http/https).";
        }
    ...
    const openConfirmToggle = (it) => {
        setConfirmState({
        type: "toggle",
        item: it,
        title: it.active !== false ? "¿Desactivar reseña?" : "¿Activar reseña?",
        message:
            it.active !== false
            ? "La reseña dejará de mostrarse en el sitio público."
            : "La reseña volverá a estar disponible para mostrarse.",
        confirmLabel: it.active !== false ? "Sí, desactivar" : "Sí, activar",
        tone: "secondary",
        });
    };
    ...
    const onConfirmAction = async () => {
        ...
        if (confirmState.type === "toggle") {
            ...
            showToast(
            "success",
            it.active !== false ? "Reseña desactivada" : "Reseña activada"
            );
        }
    ...
    return (
        ...
        <header className={styles.header}>
            ...
            <p className={styles.subtitle}>
                CRUD completo + activar/desactivar desde el panel.
            </p>
            </div>
        </header>
        ...
        <section className={styles.grid}>
            ...
            <form className={styles.form} onSubmit={onSubmit}>
                ...
                <label className={styles.field}>
                    ...
                    />
                    {errors.authorName ? (
                    <span className={styles.err}>{errors.authorName}</span>
                    ) : null}
                </label>
                ...
                <label className={styles.field}>
                    ...
                    {errors.rating ? <span className={styles.err}>{errors.rating}</span> : null}
                </label>
                </div>
                ...
                <label className={styles.field}>
                    ...
                    {errors.relativeTime ? (
                    <span className={styles.err}>{errors.relativeTime}</span>
                    ) : null}
                </label>
                ...
                {errors.authorPhotoUrl ? (
                    <span className={styles.err}>{errors.authorPhotoUrl}</span>
                ) : null}
                </label>
                ...
                />
                {errors.text ? <span className={styles.err}>{errors.text}</span> : null}
                </label>
        ...
        {confirmState ? (
            <div className={styles.modalOverlay} onClick={closeConfirm} role="presentation">
            <div
                className={styles.modalCard}
                onClick={(e) => e.stopPropagation()}
                role="dialog"
                aria-modal="true"
                aria-labelledby="google-review-confirm-title"
            >
                <div className={styles.modalBadgeWrap}>
                <Badge variant={confirmState.tone === "danger" ? "orange" : "blue"}>
                    Confirmación
                </Badge>
                </div>

                <div className={styles.modalHead}>
                <h3 id="google-review-confirm-title" className={styles.modalTitle}>
                    {confirmState.title}
                </h3>

                <p className={styles.modalText}>{confirmState.message}</p>
                <p className={styles.modalText}>
                    <b>{confirmState.item.authorName}</b>
                </p>
                </div>
